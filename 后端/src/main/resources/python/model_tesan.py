...
class TESAN(nn.Module):
    ...
    def forward(self, input, word_vecs, mask, compute_loss=False, avg_loss=True):
        ...
        out = F.log_softmax(pre_vec, dim=1) # 进行log——softmax计算，沿num_class维度dim=1
        # out是一个二维张量，第一维度是batch_size(20)，第二维度是num_class(6)
        # 对于测试数据（246），按照20一组进行分批，共13批，每批20个，最后一批只有6个
